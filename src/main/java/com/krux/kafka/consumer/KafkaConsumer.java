...
public class KafkaConsumer {
    ...
    //
    // OptionSpec<String> consumerGroupName = parser.accepts("group.id",
    // "Consumer group name.").withRequiredArg()
    // .ofType(String.class);
    ...
    // .accepts("socket.receive.buffer.bytes",
    // "The socket receive buffer for network requests").withRequiredArg()
    ...
    // .accepts("parse.messages.as.json",
    // "Toggle for parsing messages as JSON").withRequiredArg()
    ...
    // .accepts("rebalance.backoff.ms",
    // "Backoff time between retries during rebalance.").withRequiredArg()
    ...
    // .accepts("message.log.dir",
    // "Absolute path to directory under which files will be logged.")
    // .withRequiredArg().ofType(String.class);
    //
    ...
    // optionSpecs.put("zookeeper.session.timeout.ms",
    ...
    // .withRequiredArg().ofType(Integer.class).defaultsTo(6000);
    // optionSpecs.put("zookeeper.connection.timeout.ms",
    // zookeeperConnectionTimeoutMs);
    //
    // OptionSpec<Integer> zookeeperSyncTimeMs = parser
    // .accepts("zookeeper.sync.time.ms",
    // "How far a ZK follower can be behind a ZK leader").withRequiredArg()
    // .ofType(Integer.class).defaultsTo(2000);
    ...
    // if (!options.has(topicThreadMapping) || !options.has(consumerGroupName)
    // || !options.has(zookeeperUrl)
